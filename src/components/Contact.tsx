      ...
      <div className="container">
        <div className="grid lg:grid-cols-2 gap-20 relative">
          ...
          >
            ...
            <form className="flex flex-col gap-4.5" onSubmit={(e) => { e.preventDefault(); alert('Request sent!'); }}>
              <div className="grid sm:grid-cols-2 gap-4">
                <FormGroup label="First Name" placeholder="Jane" />
                <FormGroup label="Last Name" placeholder="Doe" />
              </div>
              <FormGroup label="Email Address" type="email" placeholder="you@brand.com" />
              <FormGroup label="Phone (Optional)" placeholder="(___) ___-____" />
              
              <div className="flex flex-col gap-2">
                <label className="font-mono text-[11px] tracking-[0.1em] text-paper/60 uppercase">What do you need?</label>
                <select className="bg-transparent border-b border-paper/20 py-2.5 text-[15px] focus:outline-none focus:border-amber cursor-pointer">
              ...
              <button type="submit" className="btn btn-amber mt-3 w-full justify-center group uppercase tracking-[0.08em] font-bold">
                Send Request <ArrowRight size={14} className="transition-transform group-hover:translate-x-[3px]" />
              </button>
            </form>
          </motion.div>
        </div>
